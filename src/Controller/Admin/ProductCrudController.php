...
class ProductCrudController extends AbstractCrudController
{
    ...
    public function configureCrud(Crud $crud): Crud
    {
        return $crud->setPageTitle(Crud::PAGE_INDEX, '<h1>Liste des produits</h1>')
            ...
            ->setPageTitle(
                ...
                fn(
                    Product $product
                ) => "<h1>Modification du produit <span class='text-warning'>{$product->getTitle()}</span></h1>"
            );
    }
    ...
    {
        return $actions
            ->update(Crud::PAGE_INDEX, Action::NEW, fn(Action $action) => $action->setIcon('fa fa-plus')->setLabel('Nouveau produit'))
    ...
    public function configureFields(string $pageName): iterable
    {
        return [
            ...
            ImageField::new('thumbnail', 'Miniature')->setUploadDir(
                'assets/img/upload/product_thumbnail'
            ),
        ];
    }
}
